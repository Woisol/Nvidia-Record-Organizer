		...
		<>
			...
			<div className="w-full h-[calc(100vh-32px)] px-5 py-4 relative overflow-y-scroll select-none flex flex-col">
				{recordData.length === 0 ?
					...
					:
					<>
						{recordData.map((recordData, index) => <RecordsGroup key={index} curDir={curDir} displaySize={displaySize} handleDetailWinOpen={handleDetailWinOpen} recordData={recordData} />)}
