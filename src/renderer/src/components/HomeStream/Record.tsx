				...
				{curDir === '' || RecordData.name === '' ? <p className="w-full text-center text-2xl"><br />数据不能为空！</p> :
					<Tooltip title={RecordData.name} arrow TransitionComponent={Grow} TransitionProps={{ timeout: 300 }} enterDelay={1000} classes={{ tooltip: "whitespace-nowrap" }}>
						<div className="w-full h-full">
							{/* //!为了修子元素必须是单个React元素而添加了div……且必须要设w-full */}
							<img loading="lazy" decoding="async" src={`${thumbnailDir}\\${RecordData.name.replace(".png", "_thumbnail.jpg")}`} alt={RecordData.name} className="w-full h-full object-cover" onClick={(e) => handleDetailWinOpen(e.target as HTMLImageElement, true)} />
